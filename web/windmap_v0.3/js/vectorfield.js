...
VectorField.circle = function(maxspeed,bounds,params) {
    ...
    //console.log(params);
    //
    field.getValue = function(u,v, vel,pos) { // returns value of field in real space x-y, vel is velocity in u,v space, pos is x,y
        var r = 2*(u-0.5);
        ...
        if (pos) { //pos = params.center.plus(Vector.polar(params.innerRadius+2*(u-0.5)*params.width,v*2*Math.PI));
                    //console.log(params);
            //console.log(params.sense*v*2*Math.PI);
                   pos.x = params.center.x + (params.innerRadius+2*(u-0.5)*params.width)*Math.cos(params.sense*v*2*Math.PI);
                   pos.y = params.center.y + (params.innerRadius+2*(u-0.5)*params.width)*Math.sin(params.sense*v*2*Math.PI);
                 }
