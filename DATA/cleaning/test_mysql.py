...
class DB:
    ...
    cnx = None
    def __init__(self):
        self.cnx = mysql.connector.connect(user='root',database='sonar',port=3306, password='bsccns01')
...
if __name__ == '__mainiAA__':    
    ...
    start=time.time()
    batchsize=32
    total = len(macs)-len(macs)%batchsize #We will miss some records at the end
    p=Pool(processes=int(sys.argv[1]))
    fo=open('total_times_x_mac.csv','w')
    for batchidx in range(total/batchsize):
        test=macs[batchidx*batchsize:(batchidx+1)*batchsize]
        out = p.map(isLong,test)
        for mac,val in out:
            fo.write(str(mac)+', '+str(val)+'\n')
        print ("Done "+str(batchidx*batchsize)+" out of "+str(total))
    fo.close()
    print ("Processing took "+str(time.time()-start)+" secs")
    print ("RESULT:")
    #print (out)
